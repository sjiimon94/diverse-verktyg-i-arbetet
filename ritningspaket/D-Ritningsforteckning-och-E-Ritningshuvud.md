# D. RITNINGSFÖRTECKNING

| Ritn.nr | Benämning                              | Skala     | Format | Status    | Rev. |
|---------|----------------------------------------|-----------|--------|-----------|------|
| A-01    | Planritning                            | 1:50      | A2     | Preliminär| -    |
| A-02.1  | Fasad 1 (mot befintligt hus, hög sida) | 1:50      | A3     | Preliminär| -    |
| A-02.2  | Fasad 2 (mot trädgård, låg sida)       | 1:50      | A3     | Preliminär| -    |
| A-02.3  | Fasad 3 (gavel vänster)                | 1:50      | A3     | Preliminär| -    |
| A-02.4  | Fasad 4 (gavel höger / utskjutande del)| 1:50      | A3     | Preliminär| -    |
| A-03    | Sektion A-A                            | 1:20      | A3     | Preliminär| -    |
| K-01    | Konstruktionsöversikt stomme           | 1:50      | A2     | Preliminär| -    |
| K-02    | Väggregelritning                       | 1:50      | A2     | Preliminär| -    |
| K-03    | Takregelplan                           | 1:50      | A3     | Preliminär| -    |
| K-04    | Konstruktionsdetaljer, samling         | 1:10/1:5  | A3     | Preliminär| -    |
| D-01    | Detalj D1: Anslutning tak/vägg mot bef. hus | 1:5 | A4     | Preliminär| -    |
| D-02    | Detalj D2: Yttervägg mot grund/syll    | 1:5       | A4     | Preliminär| -    |
| D-03    | Detalj D3: Takfot låg sida             | 1:5       | A4     | Preliminär| -    |
| D-04    | Detalj D4: Hörndetalj yttervägg        | 1:5       | A4     | Preliminär| -    |
| D-05    | Detalj D5: Fönster-/dörranslutning     | 1:5       | A4     | Preliminär| -    |

**Totalt antal ritningar:** 15 st

**Tillhörande SVG-exporter i paketet:** A-01 plan, A-02 fasader och A-03 sektion.

---

# E. RITNINGSHUVUD / TITLE BLOCK

Följande ritningshuvud ska användas konsekvent på samtliga ritningar:

| Fält               | Innehåll                                          |
|--------------------|---------------------------------------------------|
| **Projekt**        | Tillbyggnad träregelstomme med pulpettak           |
| **Ritningens namn**| *[Fylls i per ritning, t.ex. "Planritning"]*       |
| **Ritningsnummer** | *[Fylls i per ritning, t.ex. "A-01"]*              |
| **Skala**          | *[Fylls i per ritning, t.ex. "1:50"]*              |
| **Datum**          | 2026-07-04                                         |
| **Revidering**     | –                                                  |
| **Ritad av**       | *[Beställarens namn / CAD-projektör]*              |
| **Granskad av**    | *[Ej granskad – PRELIMINÄR HANDLING]*              |
| **Beställare**     | *[FYLLS I]*                                        |
| **Status**         | Preliminär handling                                |
| **Filnamn**        | *[Fylls i, t.ex. "A-01_Planritning.dwg"]*         |
| **Format**         | *[Fylls i per ritning, t.ex. "A2"]*                |
| **Sida**           | *[Fylls i, t.ex. "1 (15)"]*                       |
| **Projektnummer**  | *[FYLLS I]*                                        |
| **Ansvarig**       | *[FYLLS I]*                                        |
| **Kontroll**       | *[Ej kontrollerad – PRELIMINÄR HANDLING]*          |

### Revisionsruta

| Rev. | Datum | Ändring | Sign. |
|------|-------|---------|-------|
| –    | –     | Första utgåva | –  |
|      |       |         |       |
|      |       |         |       |

---
*Ritningshuvudet ska placeras i nedre högra hörnet på varje ritning.*
